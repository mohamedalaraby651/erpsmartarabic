...
import { 
  ArrowRight, Truck, ClipboardList, Paperclip, AlertTriangle, Info,
  CreditCard, Package, Star, Activity, ShoppingCart, Loader2,
} from "lucide-react";
...
const SupplierDetailsPage = () => {
  ...
  const { data: supplier, isLoading: loadingSupplier } = useQuery({
    ...
    queryFn: async () => {
      ...
      return data as (Supplier & { supplier_type?: string | null; category?: string | null; bank_name?: string | null; bank_account?: string | null; iban?: string | null; rating?: number | null; website?: string | null }) | null;
    },
    enabled: !!id,
  });
  ...
  const { data: purchaseOrders = [], isLoading: loadingOrders } = useQuery({
    ...
    queryFn: async () => {
      if (!id) return [];
      const { data, error } = await supabase.from('purchase_orders').select('*').eq('supplier_id', id).order('created_at', { ascending: false });
      if (error) throw error;
  ...
  const updateRatingMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['supplier', id] }); toast({ title: "تم تحديث التقييم بنجاح" }); },
    onError: (error) => { logErrorSafely('SupplierDetailsPage', error); toast({ title: "حدث خطأ", description: getSafeErrorMessage(error), variant: "destructive" }); },
  });
  ...
  const handlePrintStatement = async () => {
    ...
    try {
      const { data: payments, error: paymentsError } = await supabase.from('supplier_payments').select('*').eq('supplier_id', id).order('payment_date', { ascending: false });
      if (paymentsError) throw paymentsError;
      interface StatementItem { date: string; type: string; reference: string; debit: number; credit: number; status: string; }
      const statementData: StatementItem[] = [];
      purchaseOrders.forEach(order => { statementData.push({ date: order.created_at, type: 'أمر شراء', reference: order.order_number, debit: Number(order.total_amount), credit: 0, status: order.status === 'approved' ? 'معتمد' : order.status === 'pending' ? 'معلق' : order.status }); });
      interface SupplierPaymentRow { id: string; payment_date: string; payment_number: string; amount: number; }
      (payments as SupplierPaymentRow[] || []).forEach(payment => { statementData.push({ date: payment.payment_date, type: 'دفعة', reference: payment.payment_number, debit: 0, credit: Number(payment.amount), status: 'مسدد' }); });
      ...
      const dataWithBalance = statementData.map(item => { const balance = runningBalance; runningBalance = runningBalance - item.debit + item.credit; return { ...item, date: new Date(item.date).toLocaleDateString('ar-EG'), debit: item.debit > 0 ? item.debit.toLocaleString() : '-', credit: item.credit > 0 ? item.credit.toLocaleString() : '-', balance: balance.toLocaleString() }; });
      await generatePDF({ title: `كشف حساب المورد: ${supplier.name}`, data: dataWithBalance, columns: [{ key: 'date', label: 'التاريخ' }, { key: 'type', label: 'النوع' }, { key: 'reference', label: 'المرجع' }, { key: 'debit', label: 'مدين' }, { key: 'credit', label: 'دائن' }, { key: 'balance', label: 'الرصيد' }, { key: 'status', label: 'الحالة' }], includeCompanyInfo: true, includeLogo: true, orientation: 'landscape' });
      toast({ title: "تم تصدير كشف الحساب بنجاح" });
    } catch (error) {
      logErrorSafely('SupplierDetailsPage', error); toast({ title: "حدث خطأ", description: getSafeErrorMessage(error), variant: "destructive" });
    } finally { setIsPrintingStatement(false); }
  };

  if (loadingSupplier) return <DetailPageSkeleton variant="default" tabCount={5} />;
  if (!supplier) {
    return (
      <div className="flex flex-col items-center justify-center py-12">
        <Truck className="h-12 w-12 text-muted-foreground/50 mb-4" /><p className="text-muted-foreground mb-4">لم يتم العثور على المورد</p>
  ...
  const mobileStats = [
    { icon: ShoppingCart, value: totalOrders, label: 'أوامر الشراء', color: 'text-primary' },
    { icon: Package, value: `${totalPurchases.toLocaleString()}`, label: 'إجمالي المشتريات', color: 'text-info' },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {hasHighBalance && <Alert variant="destructive"><AlertTriangle className="h-4 w-4" /><AlertTitle>تنبيه: رصيد مرتفع</AlertTitle><AlertDescription>الرصيد الحالي للمورد ({currentBalance.toLocaleString()} ج.م) يتجاوز الحد المسموح به.</AlertDescription></Alert>}
        {pendingOrders.length > 0 && <Alert><Info className="h-4 w-4" /><AlertTitle>طلبات معلقة</AlertTitle><AlertDescription>يوجد {pendingOrders.length} أمر شراء في انتظار المراجعة أو الاعتماد.</AlertDescription></Alert>}
      </div>

      {/* Stats */}
      {isMobile ? <MobileStatsScroll stats={mobileStats} /> : (
        <SupplierStatsCards totalOrders={totalOrders} totalPurchases={totalPurchases} currentBalance={currentBalance} averageOrderValue={averageOrderValue} lastOrderDate={lastOrderDate} hasHighBalance={hasHighBalance} />
      )}
      ...
      <Tabs defaultValue="info" className="mt-6">
        <ScrollArea className="w-full">
          <TabsList className="flex w-max h-auto gap-1 bg-muted/50 p-1">
            ...
            <TabsTrigger value="orders" className="gap-1.5 whitespace-nowrap text-xs px-2.5"><ShoppingCart className="h-3.5 w-3.5" />أوامر الشراء ({totalOrders})</TabsTrigger>
        ...
        <TabsContent value="orders" className="mt-6">
          <Card>
            ...
            <CardContent>
              {loadingOrders ? <p className="text-muted-foreground text-center py-8">جاري التحميل...</p> : purchaseOrders.length > 0 ? (
                <Table>
                ...
                <div className="text-center py-8"><ClipboardList className="h-12 w-12 text-muted-foreground/50 mx-auto mb-4" /><p className="text-muted-foreground">لا توجد أوامر شراء</p><Button onClick={handleCreatePurchaseOrder} variant="outline" size="sm" className="mt-4">إنشاء أول أمر شراء</Button></div>
              )}
            </CardContent>
          </Card>
        </TabsContent>

        <TabsContent value="payments" className="mt-6"><SupplierPaymentsTab supplierId={id!} onAddPayment={() => setPaymentDialogOpen(true)} /></TabsContent>
        <TabsContent value="products" className="mt-6"><SupplierProductsTab supplierId={id!} /></TabsContent>
        <TabsContent value="rating" className="mt-6"><SupplierRatingTab supplierId={id!} currentRating={supplier.rating || 0} onRatingChange={(rating) => updateRatingMutation.mutate(rating)} /></TabsContent>
        <TabsContent value="activity" className="mt-6"><SupplierActivityTab supplierId={id!} /></TabsContent>
