...
export const ShimmerSkeleton = React.forwardRef<HTMLDivElement, ShimmerSkeletonProps>(
...
export const ShimmerCardSkeleton = React.forwardRef<HTMLDivElement, { className?: string }>(
  ({ className }, ref) => {
    return (
      <div ref={ref} className={cn('p-4 rounded-xl border bg-card space-y-3', className)}>
...
ShimmerCardSkeleton.displayName = 'ShimmerCardSkeleton';

// List skeleton with shimmer
export const ShimmerListSkeleton = React.forwardRef<HTMLDivElement, { count?: number; className?: string }>(
  ({ count = 5, className }, ref) => {
    return (
...
ShimmerListSkeleton.displayName = 'ShimmerListSkeleton';

// Stats skeleton with shimmer
export const ShimmerStatsSkeleton = React.forwardRef<HTMLDivElement, { count?: number; className?: string }>(
  ({ count = 4, className }, ref) => {
    return (
      <div ref={ref} className={cn('flex gap-3 overflow-x-auto pb-2 scrollbar-hide -mx-4 px-4', className)}>
        {Array.from({ length: count }).map((_, i) => (
          <div 
            key={i} 
...
ShimmerStatsSkeleton.displayName = 'ShimmerStatsSkeleton';

// Detail page skeleton with shimmer
export const ShimmerDetailSkeleton = React.forwardRef<HTMLDivElement, { className?: string }>(
  ({ className }, ref) => {
    return (
...
ShimmerDetailSkeleton.displayName = 'ShimmerDetailSkeleton';
