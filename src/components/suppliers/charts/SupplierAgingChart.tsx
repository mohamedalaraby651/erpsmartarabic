...
interface SupplierAgingChartProps {
  supplierId: string;
}
...
export function SupplierAgingChart({ supplierId }: SupplierAgingChartProps) {
  ...
  if (chartPieces.length === 0) {
    return (
      <Card>
        <CardHeader><CardTitle className="text-base">توزيع أعمار المستحقات</CardTitle></CardHeader>
        <CardContent><p className="text-center py-8 text-muted-foreground text-sm">لا توجد مستحقات</p></CardContent>
      </Card>
    );
  }
